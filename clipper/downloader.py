...
def download_youtube(url: str, dest_dir: str | Path, progress_cb=None,
    ...
    # YouTube 18+: возрастной гейт — отдельный честный текст (нужны cookies, НЕ VPN).
    if "confirm your age" in last_ytdlp_err.lower() or "age-restricted" in last_ytdlp_err.lower():
        raise RuntimeError(
            "YouTube 18+: ролик возрастной — не скачать без СВЕЖИХ cookies от залогиненного "
            "и возрастно-подтверждённого аккаунта. Текущие cookies не пробивают (истекли или "
            "аккаунт без 18+). Что делать: 1) перезалей cookies.txt из браузера, где ты ВОШЁЛ "
            "в 18+ аккаунт (из приватного окна, сразу после входа, и НЕ открывай эту вкладку "
            "после экспорта — иначе токен протухнет); 2) большинство роликов не 18+ — "
            "пропусти этот.")
    raise RuntimeError(
